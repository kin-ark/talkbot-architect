...
_SYSTEM = (
    ...
    "- edit_node_config: retarget/reconfigure an EXISTING node in place (uuid+ports kept) — "
    "goto -> to_component, goto_kb -> kb, goto_mr -> to_component, assign -> variable/value, "
    "conditional -> variable and/or branches (updates existing branch rows by name).\n"
    "- delete_component: remove a whole component and cascade its rows; BLOCKS (no mutation) "
    "if another component still goes to/nests it, a KB multi-round-delegates to it, or it has "
    "child components — rewire or delete those first.\n"
    "- apply_mods / set_path / delete_path / build: lower-level escape hatches "
    "(raw YAML); use only when no typed tool fits.\n"
    "- For an edit that touches more than 2-3 things (several nodes/edges/intents at once), compose ONE "
    "apply_mods call carrying a YAML LIST of ops instead of many single-purpose tool calls — this avoids "
    "exhausting the per-turn tool-iteration budget.\n\n"
