...
_SYSTEM = (
    ...
    "- STRUCTURE THE BOT AS MANY SMALL COMPONENTS, never one big component. Split by "
    "responsibility (Greeting, Verify Identity, Inform, the core interaction, Objection/Q&A, "
    "Closing, Wrong Number, Fallback). Real production bots use ~12 components at ~5 nodes each "
    "(0 of 33 use a single component). Target <=~8 nodes per component; if bigger, split it.\n"
    "- Map each funnel stage to its OWN component and wire them together with goto (goto_component). "
    "Extract reusable subflows (a closing, an ID check, a persuasion loop) into nested components.\n"
    "- Use multi-round components for real back-and-forth (negotiation, clarification, multi-turn Q&A); "
    "put recurring questions/objections in intent-triggered KBs, NOT deep branch chains.\n"
    ...
    "branch_intents ONLY for NON-system, intent-based routes (e.g. ServiceIssue, WantsRefund) on the SOURCE "
    "talk node: config.branch_intents = {Label: [customIntentName, ...]}. Then ALL of these must hold or the "
    "build errors: (a) each custom Label differs from the five system names; (b) every intent named is ALSO "
    "declared in custom_intents; (c) that node ALSO has a connected Unclassified edge; (d) any edge's branch is "
    "either a system name or a branch_intents label declared on its source node — a custom branch used on an "
    "edge but not declared is rejected. If you don't need intent-based routing, use ONLY the five system branches.\n"
    ...
    "- Record call outcomes as disposition tags: declare a `Disposition` category and tag each terminal/branch node with the outcome it represents (e.g. PTP, Refused, Wrong Number).\n\n"
