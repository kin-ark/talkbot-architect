...
async def export_current(s: Session = Depends(_require_session)):
    return _download(*exporters.export_full(s))
...
async def export_component(uuid: str | None = None, s: Session = Depends(_require_session)):
    ...
    if uuid is not None:
        try:
            return _download(*exporters.export_single_component(data, uuid))
        except KeyError:
            raise HTTPException(status_code=404, detail="unknown component")
    return _download(*exporters.export_components_zip(data))
