...
def test_new_creates_identified_active(tmp_path, monkeypatch):
    ...
    assert persistence.read_active() == a.id


def test_activate_swaps_contents_same_object(tmp_path, monkeypatch):
    _isolate(tmp_path, monkeypatch)
    store = SessionStore()
    a = store.new(name="A"); a.load({"BizSpeechComponent": []}); a.name = "A"; persistence.save_session(a)
    a_id = a.id
    b = store.new(name="B"); b.load({"BizSpeechComponent": []}); b.name = "B"; persistence.save_session(b)
...
def test_list_and_delete_active_falls_back(tmp_path, monkeypatch):
    _isolate(tmp_path, monkeypatch)
    store = SessionStore()
    a = store.new(name="A"); a.load({"BizSpeechComponent": []}); persistence.save_session(a)
    b = store.new(name="B"); b.load({"BizSpeechComponent": []}); persistence.save_session(b)  # b active
