...
export default function SidePanel({ summary, findings, selectedNode, onSelectNode }) {
  ...
  return (
    <div className="w-[44rem] max-w-[50%] h-full border-l border-border bg-surface flex flex-col" data-testid="side-panel">
      <div className="flex border-b border-divider">
        {TABS.map((t) => (
          <button key={t} onClick={() => { setTab(t); setDrill(null); }}
            className={`px-4 py-2 text-sm ${tab === t ? 'border-b-2 border-primary text-primary' : 'text-text-secondary'}`}>{t}</button>
        ))}
      </div>
